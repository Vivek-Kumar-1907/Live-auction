...
export default function ViewCard({ imageurl, title, description, startingBid, createdAt, createdBy, auctionLength }: {
  imageurl: string;
  title: string;
  description: string;
  startingBid: number;
  ...
  const expires = new Date(new Date(createdAt).getTime() + ( Number(auctionLength)) * 60 * 60 * 1000);
...
console.log("auctionLength raw:", auctionLength);
console.log("expires:", new Date(new Date(createdAt).getTime() + Number(auctionLength) * 60 * 60 * 1000));
  return (
    <div className="w-full rounded-lg border border-gray-200 dark:border-zinc-700 overflow-hidden bg-white dark:bg-zinc-900">
      ...
      <div className="px-4 py-3 flex flex-col gap-2">
        ...
        {createdBy && <p className="text-xs text-zinc-400">by {createdBy}</p>}
